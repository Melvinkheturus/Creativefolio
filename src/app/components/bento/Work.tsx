...
export default function Work() {
  ...
  return (
    <motion.div
      ref={cardRef}
      className="bg-[#0a0912]/60 backdrop-blur-xl border border-white/5 rounded-2xl p-4 md:p-5 shadow-lg overflow-hidden relative shimmer-wrapper"
          ...
          {projects.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                index === activeIndex ? 'bg-purple-500 w-4' : 'bg-gray-600'
              }`}
              onClick={() => {
...
const ProjectCard = React.memo(function ProjectCard({ project, delay = 0, index, total, isHovered }: ProjectCardProps) {
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });
  const cardRef = useRef<HTMLDivElement>(null);
